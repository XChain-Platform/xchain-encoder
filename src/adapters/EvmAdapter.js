 ...
 * a live route. The account-chains support spec recommends building NOTHING for
 * EVM pre-launch; this stub is the "scope + finality gate" pre-work called for in
 * spec §12 / ledger item , not the adapter build (spec §10 P0-P7).
 *
 * When the adapter IS built (post-launch, per spec), each method below replaces
 ...
 * (spec §7, do not resolve them here):
 *   - Obfuscation key: derive from (sender_address || nonce), both known
 *     pre-signing and recoverable on-chain (§7.1). deriveObfuscationKey() is the
 *     home for that, but the scheme must be decided + documented in PROTOCOL.md
 *     first, so it stays a throw.
 *   - Data carrier: calldata to a canonical sink vs self-send (§7.2).
 *   - Address canonicalization: EIP-55 vs lowercase before the ledger (§7.4).
 *
 ********************************************************************/
...
class EvmAdapter extends ChainAdapter {
    ...
    // rather than at first gate check. Defaults to the spec's primary L2 target.
    constructor(chain){
        super('evm');
    ...
    // The finality/confirmation gate for THIS chain (spec §7.5, §12). Safe to
    ...
    // Guard against THE L2 TRAP (spec §12): true only when counting this chain's
    // own block depth is a sound finality signal (never for an L2).
    isL2BlockDepthSafe(){
        return evmFinality.isL2BlockDepthSafe(this.chain);
    }

    // ── ChainAdapter surface (unbuilt; each throw documents its EVM path) ─────
    ...
    // nonce + eth_getBalance. (spec §6 xchain-encoder row)
    ...
    // heuristic, the deepest UTXO assumption in the platform). (spec §6 decoder)
    resolveSource(/* tx */){
        throw new AdapterNotImplementedError(this.family, 'resolveSource');
    }

    // EVM: payload = calldata after the XCHN magic prefix (§7.2).
    extractPayload(/* tx */){
        throw new AdapterNotImplementedError(this.family, 'extractPayload');
    }

    // EVM: key/IV from hash(sender_address || nonce) (§7.1). Scheme must be
    // decided + documented in PROTOCOL.md before this is built.
    ...
    // hash-linked parent chain like the decoder's existing walk-back. (spec §9.7)
    pollBlock(/* height */){
