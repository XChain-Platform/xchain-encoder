...
describe('XChainEncoder', () => {
	describe('#createTransaction', () => {
		...
		const encoder = new XChainEncoder("bitcoin-regtest", "http://127.0.0.1:8333", "rpc", "rpc", "");
			...
			const psbtResultante = await encoder.createTransaction(utxos, pubkey, customOutputs, data, exactFee, replacebyfee, outputType, changeAddress);
			...
			assert(txDataStr.substr(4) == data)
		});
		...
		const encoder = new XChainEncoder("bitcoin-regtest", "http://127.0.0.1:8333", "rpc", "rpc", "");
			...
			const psbt2Resultante = await encoder.createTransaction(utxos, pubkey, customOutputs, data, exactFee, replacebyfee, outputType, changeAddress, tx1Id, tx1Hex);
		...
		it('should create a multisign tx', async () => {
			...
			// Llamar a la función createTransaction
			console.log("Creating the first transaction with XchainEncoder")
			const psbtResultante = await encoder.createTransaction(utxos, pubkey, customOutputs, data, exactFee, replacebyfee, outputType, changeAddress, null, null, compressedPubKey);
			const psbtBase64 = psbtResultante.toBase64()
			...
			decodedData = decodedData.substr()
			
			assert(decodedData.substr(0,4) == "XCHN")
