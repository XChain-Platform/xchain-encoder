...
function registerDisabledRouteTests({ expect, express, fakeConsole, listen, installObservability, readObservabilityEnv }) {
    ...
    it('registers NO route when the flag is unset, but still hands back a registry', async function () {
        ...
        const obs = installObservability(app, { service: 'xchain-encoder', env: {}, console: fakeConsole() });
...
function registerInstrumentedRouteTests({ expect, express, fakeConsole, listen, installObservability, routeLabel }) {
    it('serves the exposition text and instruments requests when enabled', async function () {
        const app = express();
        app.get('/health', (req, res) => res.json({ ok: true }));
        const obs = installObservability(app, {
            service: 'xchain-encoder', version: '1.0.0', coin: 'BTC', network: 'regtest',
        ...
        try {
            ...
            expect(body).to.include('xchain_service_info{service="xchain-encoder",version="1.0.0",coin="BTC",network="regtest"');
...
function registerProtectedRouteTests({ expect, express, fakeConsole, listen, installObservability }) {
...
function registerEarlyRouteTests({ expect, express, fakeConsole, listen, installObservability }) {
...
module.exports = function registerRouteTests(context) {
    registerDisabledRouteTests(context);
    registerInstrumentedRouteTests(context);
    registerProtectedRouteTests(context);
    registerEarlyRouteTests(context);
};
