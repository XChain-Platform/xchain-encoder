...
function registerLocalLogTests({ expect, fakeConsole, createLogShipper, redactFields, scrubMessage, REDACTED }) {
    ...
    it('emits NDJSON with the envelope keys when LOG_FORMAT=json', function () {
        const sink = fakeConsole();
        const log = createLogShipper({ service: 'xchain-encoder', version: '9.9.9', env: { LOG_FORMAT: 'json' }, console: sink });
        log.warn('block stalled', { height: 42 });
        const rec = JSON.parse(sink.lines.warn[0]);
        expect(rec.level).to.equal('warn');
        expect(rec.service).to.equal('xchain-encoder');
...
function registerShippingConfigTests({ expect, fakeConsole, createLogShipper, readLogEnv, redactFields, scrubMessage, REDACTED }) {
...
function registerShippingBufferTests({ expect, fakeConsole, createLogShipper, Registry }) {
...
function registerTextEnvelopeTests({ expect, fakeConsole, createLogShipper }) {
    it('renders ts, lowercase level, service tag, message, then key=value', function () {
        ...
        expect(sink.lines.warn[0]).to.match(
            /^\d{4}-\d{2}-\d{2}T[\d:.]+Z warn \[xchain-encoder\] PBFT_DROP reason=digest_mismatch phase=prepare round=42$/
        );
    });
...
function registerTextRedactionTests({ expect, fakeConsole, createLogShipper, REDACTED }) {
...
function registerLogShipperTests(context) {
    registerLocalLogTests(context);
    registerShippingConfigTests(context);
    registerShippingBufferTests(context);
}

function registerTextLogTests(context) {
    registerTextEnvelopeTests(context);
    registerTextRedactionTests(context);
}

module.exports = { registerLogShipperTests, registerTextLogTests };
