...
// the same assertions against xchain-encoder's own copy, express version and
...
describe('observability/metrics: exposition format', function () {
    ...
    it('collectDefaultMetrics exposes process and service identity', function () {
        const reg = new Registry();
        collectDefaultMetrics(reg, { service: 'xchain-encoder', version: '1.2.3', coin: 'BTC', network: 'regtest' });
        const out = reg.render();
        expect(out).to.include('xchain_service_info{service="xchain-encoder",version="1.2.3",coin="BTC",network="regtest"');
