...
// like it did.
//
// Several guards in test/unit are byte-identity or value-parity checks against
// a SIBLING checkout (the vendored coins registry against xchain-hub, the
// action manifest against xchain-documentation, envelope recognition against
// the authoritative decoder, and so on). Every one of them resolves its
// sibling with existsSync and calls this.skip() when it is absent. That is the
// right behaviour for a single-repo checkout, and it is invisible: mocha
// reports a skipped test as `pending`, the suite still exits 0, and the push
// gate prints PASS. A sibling the CI venue was never told to ship
// (.ci-siblings) is therefore indistinguishable, in the output, from one whose
// guards all ran and passed.
//
// Measured 2026-08-12 against a full local checkout, a venue carrying only
// this repo's then-declared siblings ran 581 passing / 6 pending where the
// full checkout runs 584 / 3, so 3 guards had silently not run.
//
