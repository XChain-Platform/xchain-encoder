 ...
 * test/unit/openrpc_coverage.test.js fails if the two drift apart.
 *
...
const METHODS = [
    ...
    },
    {
        ...
        result: { name: 'cancel', schema: { type: 'object', properties: { psbt: str('unsigned key-path sweep PSBT, hex'), encoding: str('"TAPROOT"'), cancel: bool('always true'), fee: int('miner fee funded, base units'), reservation: { type: 'object', description: 'receipt for the commit-outpoint reservation this cancel kept: {id, outpoints, expiresAt}; pass id to release_inputs if the cancel is abandoned', properties: { id: str('reservation ticket id (32-char lowercase hex)'), outpoints: { type: 'array', items: { type: 'string' } }, expiresAt: int('epoch ms the reservation lapses at') } } } } },
    },
    {
        name: 'release_inputs',
        summary: 'Hand back the input reservations one earlier build took, named by its reservation ticket id.',
        description: 'A successful create_tx / create_envelope_cancel_tx keeps its inputs reserved for five minutes, '
            + 'because the caller is expected to sign and broadcast them. A wallet that composes when its send modal '
            + 'opens abandons most of those builds, and each abandoned one holds the address\'s inputs for the rest of '
            + 'the window, so the next compose on a few-UTXO address reports insufficient funds against its own money. '
            + 'Call this as soon as you know you will not broadcast. Ownership is enforced by the ticket: the id is '
            + 'unguessable and every outpoint is freed only while it still carries the stamp that build wrote, so a '
            + 'claim another call has since taken over is reported in `retained` and left in place. Idempotent and '
            + 'never an error: an unknown, expired or already-released ticket returns found:false with empty lists.',
        params: [{ name: 'reservationId', required: true, schema: str('reservation ticket id from result.reservation.id (32-char lowercase hex)') }],
        result: { name: 'release', schema: { type: 'object', properties: { reservationId: str('the ticket id that was presented'), found: bool('the ticket was live and has now been consumed; false for an unknown, expired or already-released ticket'), released: { type: 'array', description: 'outpoints ("txid:vout") this call freed', items: { type: 'string' } }, retained: { type: 'array', description: 'outpoints on the ticket whose reservation had already lapsed and been re-taken by another build, so they were left alone', items: { type: 'string' } } } } },
    },
    {
        name: 'broadcast_tx',
...
// without rewriting that artifact mid-run (see test/unit/openrpc_coverage.test.js).
