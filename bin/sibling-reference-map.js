 ...
 *
 *   text       any literal run of `xchain-encoder/src/<path>` in any text file:
 *              a relative require, a comment, a shell script, a markdown runbook.
 *   join       a path built segment by segment, the shape
 *              path.join(root, 'xchain-encoder', 'src', 'foo.js'). When every
 ...
 *              `const R = path.resolve(__dirname, '../../xchain-encoder')` then
 *              path.join(R, 'src', 'foo.js'), path.join(R, 'src/foo.js') or
 *              `${R}/src/foo.js`. The root may equally be
 *              process.env.XCHAIN_ENCODER_PATH, a candidate array filtered to its
 ...
 *              this checkout, `encoderFile('src/rollback.js')`. A call over a
 *              literal array, `encoderFile('src/' + twin)`, is emitted once per
 ...
 *              `copy_twin xchain-encoder "src/$f"`, resolved against the literal
...
const REPO_NAME = 'xchain-encoder';
...
// `xchain-encoder/src/<path>`, however it was spelled: a relative require
// (`../../xchain-encoder/src/utility.js`), a prose mention, a shell path.
const TEXT_REFERENCE = new RegExp(`${NAME_RE}\\/(src\\/[A-Za-z0-9_@.\\-/]+)`, 'g');

// path.join(..., 'xchain-encoder', 'src', ...): the tail is captured raw and
...
// Environment variables that hold a checkout of this repo. XCHAIN_ENCODER_SQL_PATH
...
const ENV_ROOT_VARS = ['XCHAIN_ENCODER_PATH', 'XCHAIN_ENCODER_DIR'];
...
function rootPrefix(init) {
    ...
    while ((m = re.exec(init)) !== null) {
        ...
        // path.join(base, 'xchain-encoder', 'src', 'sql').
...
function collectRootProducers(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // `const p = encoderFile('src/x.js')` look like another checkout.
 ...
 * checkout, the `encoderFile('src/rollback.js')` idiom. Recognised by a body
...
function helperReferences(text, helpers, lists, strings) {
    ...
    for (const [name, suffix] of helpers) {
        ...
        let m;
        while ((m = re.exec(text)) !== null) {
            // `function encoderFile(rel){` reads as a call of itself. Its
 ...
 * relative path, `copy_twin xchain-encoder "src/$f"`, where the file comes from
