...
function mochaArgsFor(script) {
    ...
    // encoder's scripts spawn mocha through `npx mocha ...` rather than a bare
    // `mocha ...`; npx is not the command being pinned, so it is skipped here
    // rather than counted as an unsupported runner (this repo's own fork of this tool).
    if (tokens[i] && tokens[i].value === 'npx') i += 1;
